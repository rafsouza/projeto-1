...
class Pedido(db.Model):
  ...
  preference_id = db.Column(db.String(64), nullable=True)
  payment_id =  db.Column(db.String(32), nullable=True)
...
# Construtor para pagamentos
  def __call__(self, cliente_id, N_pedido, status, data_entrega, obs, link, preference_id, payment_id):
    self.cliente_id = cliente_id
    self.N_pedido = N_pedido
    self.status = status
    self.data_entrega = data_entrega
