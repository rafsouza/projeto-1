...
def vercart():
    if 'CarrinhoLoja' not in session or len(session['CarrinhoLoja'])<=0:
        ...
        return render_template('/loja/produtos/carrinho_teste.html', title='Carrinho - Morro das Panelas', msg = 'O carrinho está vazio', categorias=cats())
    ...
    # print('Carrinho: ',session['CarrinhoLoja'].items())
    return render_template('/loja/produtos/carrinho_teste.html', title='Carrinho - Morro das Panelas', categorias=cats(), tot =tot, form=form)
        ...
        #  qtd = [a.replace(',','.') for a in qtd] # Substituir vírgula por ponto
         try:
            ...
            #  flash('Os itens do carrinho foram atualizados','success')
             ...
             for key, p in session['CarrinhoLoja'].items():
                tot += float(p['preco']) * float(p['qtd']) 
             return render_template('/loja/produtos/carrinho_teste.html', title='Carrinho - Morro das Panelas',
    ...
    try:
        ...
        for key , it in session['CarrinhoLoja'].items():
            if int(key) == id:
                ...
                for key, p in session['CarrinhoLoja'].items():
                    tot += float(p['preco']) * float(p['qtd']) 
        return render_template('/loja/produtos/carrinho_teste.html', title='Carrinho - Morro das Panelas', msg = 'Item removido do carrinho', categorias=cats(), tot=tot)
    except Exception as e:
        print(e)
    return redirect(url_for('/carrinho.vercart'))
