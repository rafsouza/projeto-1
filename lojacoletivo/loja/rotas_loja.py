...
def confirmar_pedido():
  ...
  if request.method == 'GET':
    ...
    return render_template('/loja/produtos/confirmar.html', title= 'Confirmar Pedido - Morro das Panelas', cl=cl, categorias=cats(), tot=tot, link_pagamento=link_iniciar_pagamento)

  ## Fazer checkout - 1 etapa
  ## Pegar info do admin e enviar solicitação para API do mercadopago

  ## Receber e processar resposta da API - 2 etapa - Rafael
  ## Persistir
...
def compra_pendente():
    return render_template('/loja/compra_status.html', status='pending', mensagem='Seu pagamento está pendente. Aguarde a confirmação.')
